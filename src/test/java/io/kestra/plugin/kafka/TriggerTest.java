    ...
    void flow() throws Exception {
        ...
        try (Worker worker = applicationContext.createBean(Worker.class, IdUtils.create(), 8, null)) {
            try (
                ...
                // wait for execution
                List<Execution> executions = new java.util.concurrent.CopyOnWriteArrayList<>();
                executionQueue.addListener(execution -> {
                    executions.add(execution.getLeft());
                ...
                Integer trigger = (Integer) executions.get(executions.size() - 1).getTrigger().getVariables().get("messagesCount");

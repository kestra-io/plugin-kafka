...
public class RealtimeTrigger extends AbstractTrigger implements RealtimeTriggerInterface, KafkaConnectionInterface, ConsumeInterface {
    ...
    public Publisher<Execution> evaluate(ConditionContext conditionContext, TriggerContext context) {
        ...
        return Flux.from(task.stream(runContext))
            .map((record) -> TriggerService.generateExecution(this, conditionContext, context, task.recordToMap(record)));
    }
}
